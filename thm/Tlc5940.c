/*  Copyright (c) 2009 by Alex Leone <acleone ~AT~ gmail.com>
    ...
    modifications:
    * removed the c++ class and unwrapped functions to tlc_ namespace
    * moved the Timer interrupts to this file.
*/
...
uint8_t tlc_GSData_buf[NUM_TLCS * 24];
uint8_t* tlc_GSData = tlc_GSData_buf;
...
void tlc_set(TLC_CHANNEL_TYPE channel, uint16_t value) {
    tlc_set_buf( tlc_GSData, channel, value );
}
...
uint16_t tlc_get( TLC_CHANNEL_TYPE channel ) {
    return tlc_get_buf( tlc_GSData, channel );
}
...
void tlc_setAll(uint16_t value) {
    tlc_setAll_buf(tlc_GSData, value);
}
...
void tlc_dcModeStart(void)
{
    ...
    tlc_clear_XLAT_interrupt(); // (in case this was called right after update)
    tlc_needXLAT = 0;
...
/**
 * Interrupt Handler required for TLC5940 Module
 */
ISR(TIMER1_OVF_vect)
{
    tlc_disable_XLAT_pulses();
