...
#if (MCPDEBUG_TXBUF)
			printf_P( PSTR("Selected TX-Buffer"), i+1);
#endif
...
uint8_t mcp2515_init(const uint8_t canSpeed)
{
    ...
    printf_P( PSTR("Setting mcp2515 to config mode\n\r"));
	res = mcp2515_setCANCTRL_Mode(MODE_CONFIG);
	
	if ( res == MCP2515_FAIL ) return res;  /* function exit on error */
	
    printf_P( PSTR( "Setting mcp2515 timings\n\r"));
...
//			MCP_RXB_RX_STDEXT);
        ...
        // We're using only std identifier
        mcp2515_modifyRegister(MCP_RXB0CTRL,
            MCP_RXB_RX_MASK | MCP_RXB_BUKT_MASK, // receive standard messages
            MCP_RXB_RX_STD );    // and rollover

        mcp2515_modifyRegister( MCP_RXB1CTRL,
            MCP_RXB_RX_MASK | MCP_RXB_BUKT_MASK, // receive standard messages
            MCP_RXB_RX_STD ); // enable roll-over
#endif
	}
...
void mcp2515_dumpExtendedStatus(void)
{
	...
	printf_P( PSTR("MCP2515 Extended Status:\n\r"));
	printf_P( PSTR("MCP Transmit Error Count = %d\n\r"), tec);
	printf_P( PSTR("MCP Receiver Error Count = %d\n\r"), rec);
	printf_P( PSTR("MCP Error Flag = 0x%x"), eflg);
	
	if ( (rec>127) || (tec>127) ) {
		printf_P( PSTR("Error-Passive or Bus-Off\n\r"));
	}

	if (eflg & MCP_EFLG_RX1OVR) 
		printf_P( PSTR("Receive Buffer 1 Overflow\n\r"));
	if (eflg & MCP_EFLG_RX0OVR) 
		printf_P( PSTR("Receive Buffer 0 Overflow\n\r"));
	if (eflg & MCP_EFLG_TXBO) 
		printf_P( PSTR("Bus-Off\n\r"));
	if (eflg & MCP_EFLG_TXEP) 
		printf_P( PSTR("Receive Error Passive\n\r"));
	if (eflg & MCP_EFLG_TXWAR) 
		printf_P( PSTR("Transmit Error Warning\n\r"));
	if (eflg & MCP_EFLG_RXWAR) 
		printf_P( PSTR("Receive Error Warning\n\r"));
